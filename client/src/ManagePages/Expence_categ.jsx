...
const Expence_categ = () => {
  ...
  return (
    ...
    <Box style={{width:'50%', margin:'auto'}}>
        ...
        <Box>
          <Accordion defaultIndex={[1]} allowMultiple borderBlock='white'>
              <AccordionItem >
                <h2>
                  <AccordionButton>
                  <Box w='150px' h='40px' bg='green' color='white' textAlign='center' padding='7px 15px' borderRadius='10px'>
                    <button >+ New category</button>
                  </Box>
                  </AccordionButton>
                </h2>
                <AccordionPanel pb={4} style={{border:'1px solid red'}}>
                  <Box display='flex' flexDirection='column' gap='25px' padding='10px'>
                    <Text color='black' fontWeight='600'>Category Name</Text>
                    <Input value={category_name} onChange={(e) => setCateg_name(e.target.value)}/>
                    <Box >
                      <Button width='140px' bg='green'color='white' mr={5} padding='0px 35px' onClick={addcateg} _hover={{bg:'green'}}>
                        Save category
                      </Button>
                      <Button width='100px' border='1px solid ' bg='white' color='black' mr={5} padding='0px 35px'  _hover={{bg:'white'}} onClick={cancelcateg}>
                        Cancel
                      </Button>
                    </Box>
        ...
        {categ.map((item) =>(
          <Box border='1px solid grey' borderLeft='none' borderRight='none' height='50px' display='flex' justifyContent='space-between' marginTop ='20px'>
            <Box display='flex' gap='30px' padding='10px'>
              {/* <Link to={`${item._id}/edit`}> */}
                <button style={{border:'1px solid grey',padding:'0px 10px',borderRadius:'5px',background:'white',textAlign:'center'}}>Edit</button>
              {/* </Link> */}
              <Text fontWeight='bold' fontSize='17px'>{item.category_name}</Text>
            </Box>
